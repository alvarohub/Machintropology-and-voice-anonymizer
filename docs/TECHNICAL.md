...
- **Python 3.11** (tested with conda env `ML311`)
...
pip install opensmile   # strongly recommended
```

The `requirements.txt` installs: numpy, sounddevice, matplotlib, funasr, torch, torchaudio, modelscope, pyworld.

**openSMILE** is optional but recommended — without it, prosody falls back to pyworld (basic F0 + energy only).
...
On first run, emotion2vec_plus_base (~350 MB) downloads from ModelScope/HuggingFace. Subsequent runs use the cached model.
