...
class RadarDisplay:
    ...
    def __init__(
        ...
        # Defaults for silence (use lower bound of ylim)
        self.pros_defaults = {k: yl[0] for k, yl in zip(self.pros_keys, self.pros_ylims)}
        ...
        # Left column: radar on top, then sliders + VAD toggle
        gs_left = gridspec.GridSpecFromSubplotSpec(
            4, 1, subplot_spec=gs_top[0],
            height_ratios=[1, 0.04, 0.04, 0.06], hspace=0.25,
        )
        self.ax_radar = self.fig.add_subplot(gs_left[0], polar=True)
        self._setup_radar()

        # Window-size slider
        self._slider_ax = self.fig.add_subplot(gs_left[1])
        ...
        self._vad_slider_ax = self.fig.add_subplot(gs_left[2])
        ...
        self._vad_btn_ax = self.fig.add_subplot(gs_left[3])
        ...
        self._vad_btn.label.set_fontsize(9)
    ...
    def _ingest_prosody_lld(self, msg: dict) -> None:
        ...
        for i in range(n_frames):
            ...
            for k in self.pros_keys:
                if k in frames:
                    self._pros_lld_data[k].append(float(frames[k][i]))
                else:
